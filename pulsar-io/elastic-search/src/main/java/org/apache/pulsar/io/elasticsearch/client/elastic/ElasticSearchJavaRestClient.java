...
public class ElasticSearchJavaRestClient extends RestClient {
    ...
    public ElasticSearchJavaRestClient(ElasticSearchConfig elasticSearchConfig,
                                       BulkProcessor.Listener bulkProcessorListener) throws MalformedURLException {
        super(elasticSearchConfig, bulkProcessorListener);

        URL url = new URL(config.getElasticSearchUrl());
        log.info("ElasticSearch URL {}", url);
        RestClientBuilder builder = org.elasticsearch.client.RestClient.builder(new HttpHost(url.getHost(), url.getPort(), url.getProtocol()))
